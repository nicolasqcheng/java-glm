 ...
 */
public class CombinedModelWithoutAnglesTest {
    private static final Logger logger = LoggerFactory.getLogger(CombinedModelWithoutAnglesTest.class);
 
    private static Random random = new Random();

    @Test
    public void test_combined_model_without_angles ()throws IOException {
        ...
        //Test on real data
        InputStream testStream = FileUtils.getResource("test_set_408_MBF_Sit_MG_SG_MB_SB_01-000.csv");
        ...
        String fn = "./output_test_set_408_MBF_Sit_MG_SG_MB_SB_01-000_test_data.txt";
        FileWriter fw = new FileWriter(fn);
        BufferedWriter bw = new BufferedWriter(fw);
        for(int i = 0; i < testData.rowCount(); ++i){
            double sitstand_predicted = glm_SitStand.transform(testData.row(i));
            String posture = sitstand_predicted<0.5?"sit":"stand";
            String status = "";
            double goodbad_predicted = 0;
            if (sitstand_predicted >=0.7 ) {
